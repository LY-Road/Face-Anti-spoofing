...
class FusionNet(nn.Module):
    ...
    def __init__(self, num_class=2):
        ...
        # 加载Net,实际实现为resnet18  
        # Net内实际是调用_make_layer与basikblock实现的
...
def run_check_net():
    ...
    print(net)
    #x =torch.tensor(np.zeros(1,9,32,32)).double()
    # 生成一个矩阵，模拟一个3张图片合成的一个样本
    x=torch.tensor(np.zeros([1,9,32,32])).float()
    # 输入到网络中，查看网络各层feature map
    net.forward(x)
